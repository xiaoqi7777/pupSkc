...
async function vodLevel2Response(url, resp, iptv) {
	...
	console.log('-totalCount page',data.totalCount)
	console.log('-totalCount data', data.data[1])
...
async function vodLevel1Response(url, resp, iptv) {
	...
	// logger.debug(`level 2 data: --------------`);
	logger.debug(`level 2 的数据数${level2Column}`);
	// logger.debug(level2Column);
	...
	// logger.debug('level2Column-----------',level2Column);
	//删除 监视level1的方法
