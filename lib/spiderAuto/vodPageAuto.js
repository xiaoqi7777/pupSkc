...
async function vodLevel3Response(url, resp, iptv) {
	// 获取连续剧列表
}
...
async function vodLevel1Response(url, resp, iptv) {
	...
	let data = rd[0];
	// 初次进入Level1 的时候 columnId=== null  获取 Level1 总列数
	if (columnId == 'null') {
		...
		*/
		...
		iptv.addPageProcessor(/get_vod_list.jsp?/, null, vodLevel2Response);
		iptv.addPageProcessor(/get_vod_info.jsp?/, null, vodLevel3Response);


	} else {
		// 记录当前的列 currentColumn
	...
	logger.info(`当前的列数,${level1Column} - ${columnId} - ${level1Column.length}`)
	...
	for (let i = 0; level1Column && i < level1Column.length; i++) {
		...
		if (column.columnId === columnId) {
			// 记录当前的列 currentColumn
			currentColumn = i
			logger.info(`当前的列数,${currentColumn} `)
