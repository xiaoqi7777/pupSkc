...
class Listen extends events {
	constructor(pop) {
		...
		this.isEmitNumber = 0;
		this.on('isBack',data=>{
                        console.log('isBack  jinali -------',data)
			this.isBack = data
			this.isEmitNumber = 0
		})
	}
	...
	interceptBack(data){
		if (data.url().includes('bg_index_club_new2')) {
			...
			this.io.emit('isBack',{isBack:'on'})
		}
	}
	async  isGoBack(data){
		if(this.isBack === 'back'){
			console.log('--listen-- trigger back--',)
		 // if(data.url().includes('5CYII=')|| data.url().includes('iptvepg/empty.jsp')){
		//if(data.url().includes('iptvepg/empty.jsp')){
		//	console.log('-----------------------')
		...
		if(data.url().includes('5CYII=') || this.isReload){
			this.isReload = false
			console.log('monitor error page -- execute back ')
	...
	async init() {
		...
		this.page.on('framedetached',()=>{
			...
			this.io.emit('isBack',{isBack:'ok'})
		})
		console.log('keng~~~~~~~~~~~~~`')
		// this.page.on('framedetached',()=>{
		// 	//当 iframe 从页面移除的时候触发。
		// 	onsole.log('移出 framedetached')
		// })
